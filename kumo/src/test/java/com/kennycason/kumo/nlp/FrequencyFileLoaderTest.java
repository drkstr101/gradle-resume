...
public class FrequencyFileLoaderTest {
  ...
  public void basic() throws IOException {
    final List<WordFrequency> wordFrequencies = FREQUENCY_FILE_LOADER
        .load(IOUtils.toInputStream("100:cat\n" + "50:dog\n" + "25:fish\n" + "75:frog"));
  ...
  // read valid file, and assert sorted order, ignores empty lines, spaces, and
  // tabs
  @Test
  public void validFile() throws IOException {
    final List<WordFrequency> wordFrequencies = FREQUENCY_FILE_LOADER
        .load(Thread.currentThread().getContextClassLoader().getResourceAsStream("frequency_file.txt"));
  ...
  public void malformedWord() throws IOException {
    FREQUENCY_FILE_LOADER
        .load(Thread.currentThread().getContextClassLoader().getResourceAsStream("frequency_file_malformed_word.txt"));
  }

  @Test(expected = KumoException.class)
  public void malformedWordCount() throws IOException {
    FREQUENCY_FILE_LOADER.load(
        Thread.currentThread().getContextClassLoader().getResourceAsStream("frequency_file_malformed_word_count.txt"));
  }
