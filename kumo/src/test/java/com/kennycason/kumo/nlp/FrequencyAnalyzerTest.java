...
public class FrequencyAnalyzerTest {
  ...
  public void defaultTokenizerTrim() throws IOException {
    ...
    final List<WordFrequency> wordFrequencies = frequencyAnalyzer
        .load(Thread.currentThread().getContextClassLoader().getResourceAsStream("trim_test.txt"));

    final Map<String, WordFrequency> wordFrequencyMap = wordFrequencies.stream()
        .collect(Collectors.toMap(WordFrequency::getWord, Function.identity()));
  ...
  public void noNormalizationNoFilter() throws IOException {
    ...
    final List<WordFrequency> wordFrequencies = frequencyAnalyzer
        .load(Thread.currentThread().getContextClassLoader().getResourceAsStream("text/simplymeasured.txt"));
