...
public class WordCloud {
  ...
  /**
   * create background, then draw current word cloud on top of it. Doing it this
   * way preserves the transparency of the this.bufferedImage's pixels for a more
   * flexible pixel perfect collision
   */
  protected void drawForegroundToBackground() {
    ...
    final BufferedImage backgroundBufferedImage = new BufferedImage(dimension.width, dimension.height,
  ...
  protected List<Word> buildWords(final List<WordFrequency> wordFrequencies, final ColorPalette colorPalette) {
    ...
    for (final WordFrequency wordFrequency : wordFrequencies) {
      // the text shouldn't be empty, however, in case of bad normalizers/tokenizers,
      // this may happen
  ...
  private Word buildWord(final WordFrequency wordFrequency, final int maxFrequency, final ColorPalette colorPalette) {
    ...
    final Word word = new Word(wordFrequency.getWord(), wordFrequency.getCategory() == -1 ? colorPalette.next()
        : colorPalette.getColors().get(wordFrequency.getCategory()), fontMetrics, this.collisionChecker, theta);
