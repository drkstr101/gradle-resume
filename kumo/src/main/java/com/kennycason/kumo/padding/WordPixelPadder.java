...
/**
 * Created by kenny on 7/1/14. Add pixel padding around the numbers
 */
public class WordPixelPadder implements Padder {
  ...
  public void pad(final Word word, final int padding) {
    ...
    for (int x = 0; x < width; x++) {
      ...
      pixelsSetInPaddingPerColumn[x] = countNotTransparentPixels(originalRaster, 0, Math.min(padding, height - 1), x);
    }

    for (int y = 0; y < height; y++) {
      // is the line inside the image?
      if (y - padding >= 0) {
        // the line (y - padding) is now outside the padding area, we need to update our
        // index
      ...
      if (y > 0 && y + padding < height) {
        // the line (y + padding) is now inside the padding area, we need to update our
        // index
        final int line = y + padding;
        int set = -1;
      ...
      for (int x = 0; x < width; x++) {
        if (x - padding >= 0) {
          // the column (x - padding) is now outside the padding area, we need to update
          // our counter
          pixelsSetInPaddingArea -= pixelsSetInPaddingPerColumn[x - padding];
        }
        if (x > 0 && x + padding < width) {
          // the column (x + padding) is now inside the padding area, we need to update
          // our counter
          pixelsSetInPaddingArea += pixelsSetInPaddingPerColumn[x + padding];
        }
  ...
  private int countNotTransparentPixels(final CollisionRaster originalRaster, final int minY, final int maxY,
      final int x) {
    int n = 0;
