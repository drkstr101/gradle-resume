 ...
 * from a text file.
 *
 * Sample format: 100: frog 94: dog 43: cog 20: bog 3: fog 1: log 1: pog
 */
public class FrequencyFileLoader {
  ...
  private static List<WordFrequency> processLines(final List<String> lines) {
    return lines.stream().filter(StringUtils::isNotBlank).map(FrequencyFileLoader::buildWordFrequency)
        .sorted(WordFrequency::compareTo).collect(Collectors.toList());
  }
