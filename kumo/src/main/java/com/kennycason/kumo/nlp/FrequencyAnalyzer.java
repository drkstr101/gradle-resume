...
public class FrequencyAnalyzer {
   ...
   */
  public List<WordFrequency> load(final InputStream inputStream, boolean autoFill, String autoFillWord)
      throws IOException {
  ...
  // Else if there are some words to draw, repeat them until the number of
  // characters is similar with wordFrequenciesToReturn
  // Else repeat autoFillWord to satisfy the drawing cloud need.
  // Note: the repeat time is dependence on the sum of word length. We consider
  // character number is more important.
  ...
  private Map<String, Integer> buildWordFrequencies(final List<String> texts, final WordTokenizer tokenizer) {
    return texts.stream().map(tokenizer::tokenize).flatMap(List::stream).map(this::normalize).filter(buildFilter())
  ...
  private List<WordFrequency> takeTopFrequencies(final Collection<WordFrequency> wordCloudEntities) {
    return wordCloudEntities.stream().sorted(WordFrequency::compareTo).limit(wordFrequenciesToReturn)
