...
public class RectanglePixelCollisionChecker implements CollisionChecker {
  ...
  /*
   * ax,ay ___________ax + a.width | | | | | bx, by_________|__ bx + b.width |
   * |(INTERSECTION)| | |__|______________| | ay + height |
   * |______________________| by + height
   */
  @Override
  public boolean collide(final Collidable collidable, final Collidable collidable2) {
    ...
    for (int i = stepSize - 1; i >= 0; i--) {
      for (int y = startY + i; y < endY; y += stepSize) {
        ...
        int absolute2 = position2.x
            + collisionRaster2.nextNotTransparentPixel(startX - position2.x, endX2, yOfPosition2);
        ...
        while (true) {
          if (absolute1 > absolute2) {
            absolute2 = position2.x
                + collisionRaster2.nextNotTransparentPixel(absolute1 - position2.x, endX2, yOfPosition2);

            if (absolute2 == stop2) {
              break;
            }
          } else if (absolute1 < absolute2) {
            absolute1 = position.x
                + collisionRaster.nextNotTransparentPixel(absolute2 - position.x, endX1, yOfPosition);
